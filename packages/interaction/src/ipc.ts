...
function assertLocalEndpoint(value: unknown): string {
  ...
  if (
    ...
    throw new TypeError("interaction IPC endpoint must be a Guardian temporary Unix socket");
  }
  return value;
}

export interface InteractionIpcCredentials {
  readonly endpoint: string;
  readonly capability: string;
}

export function createInteractionIpcCredentials(): InteractionIpcCredentials {
  const id = randomUUID();
  return {
    endpoint:
      process.platform === "win32"
        ? `\\\\.\\pipe\\guardian-interaction-${id}`
        : join(tmpdir(), `guardian-interaction-${id}.sock`),
  ...
  return new Promise((resolveLine, rejectLine) => {
    ...
    socket.on("data", (chunk: string) => {
      ...
      if (bytes > maximumBytes) {
        rejectOnce(new TypeError("interaction IPC frame is oversized"));
        return;
      }
      buffer += chunk;
      ...
      if (buffer.slice(newline + 1).trim().length !== 0) {
        rejectOnce(new TypeError("interaction IPC accepts exactly one frame"));
        return;
      }
      settled = true;
      resolveLine(buffer.slice(0, newline));
    });
    socket.once("end", () => rejectOnce(new TypeError("interaction IPC frame is incomplete")));
    socket.once("close", () => rejectOnce(new TypeError("interaction IPC connection closed")));
    socket.once("error", rejectOnce);
  });
}

function writeResponse(socket: Socket, response: unknown): void {
  socket.end(`${JSON.stringify(InteractionIpcResponseSchema.parse(response))}\n`);
}

export interface InteractionProviderResult {
  readonly providerRequestId: string;
  readonly outcome: InteractionOutcome;
}

export type InteractionIpcHandler = (
  context: InteractionMissionContext,
  evaluatedAt: string,
) => Promise<InteractionProviderResult>;

export class LocalInteractionIpcServer {
  ...
  constructor(
    configValue: unknown,
    handler: InteractionIpcHandler,
    options: { readonly now?: () => string } = {},
  ) {
    const config = InteractionServiceProcessConfigSchema.parse(configValue);
  ...
  async #serve(socket: Socket): Promise<void> {
    ...
    let request: InteractionIpcRequest;
    try {
      request = InteractionIpcRequestSchema.parse(
    ...
    try {
      const result = await this.#handler(this.#config.context, evaluatedAt);
      ...
      const parsed = InteractionIpcFailureReasonSchema.safeParse(
...
export class InteractionIpcError extends Error {
  readonly reason: InteractionIpcFailureReason;

  constructor(reason: InteractionIpcFailureReason) {
    super(`interaction service failed: ${reason}`);
    this.name = "InteractionIpcError";
    this.reason = reason;
  }
}

export class LocalInteractionIpcClient {
  readonly #endpoint: string;
  readonly #binding: Omit<InteractionIpcRequest, "requestedAt" | "turn">;
  ...
  constructor(options: {
    ...
    this.#binding = InteractionIpcRequestSchema.omit({ requestedAt: true, turn: true }).parse({
      schemaVersion: 1,
    ...
    if (!Number.isInteger(timeoutMs) || timeoutMs < 100 || timeoutMs > 60_000) {
      throw new TypeError(
        "interaction IPC timeout must be an integer from 100 to 60000 milliseconds",
      );
    }
    this.#timeoutMs = timeoutMs;
  }

  async runFirstTurn(requestedAtValue: unknown): Promise<InteractionProviderResult> {
    const frame = InteractionIpcRequestSchema.parse({
      ...this.#binding,
    ...
    try {
      ...
      if (!response.ok) throw new InteractionIpcError(response.error);
      return response;
    } catch (error) {
      if (error instanceof InteractionIpcError) throw error;
      throw new InteractionIpcError("provider_unavailable");
    } finally {
      socket.destroy();
    }
  }
