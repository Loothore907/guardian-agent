...
worker_source="${1:?worker source path is required}"
sandbox_root="$(mktemp -d /tmp/guardian-session.XXXXXX)"
...
printf 'guardian:x:1000:1000:Guardian Session:/workspace:/usr/bin/false\n' > "$sandbox_root/etc/passwd"
printf 'guardian:x:1000:\n' > "$sandbox_root/etc/group"
cp -- "$worker_source" "$sandbox_root/workspace/worker.py"
chmod 0555 "$sandbox_root/workspace/worker.py"
chmod 1777 "$sandbox_root/tmp"

chroot "$sandbox_root" /usr/bin/env -i \
  HOME=/workspace \
  LANG=C.UTF-8 \
  ...
  /usr/bin/python3 /workspace/worker.py
