...
Judge portal additions: untrusted objectives, targets, seeded content and model
claims cannot create authority. Local API tests cover caller/preview binding,
replay, expiry, cancellation, schema expansion and sanitized errors. Persistent
fixture reservations prevent concurrent/restarted sessions from reusing a mutation
target. Trusted backend composition and source-fingerprint limitations remain part
of the boundary. C7 local tests cover exact tool-result binding, bounded untrusted
history, durable replay/accounting, grant revocation, service failure and deadlines.
...
## Session-grant boundary

[ADR-0048](adr/0048-typed-session-plan-runtime.md) adds local controls against
reusing approval after plan revocation/exhaustion, swapping a grant during a
credential wait, replaying a merge under a new request ID, or treating a pending
proposal as consent. Only the authorization role stores/replaces/revokes grants;
model recommendations cannot lower the floor. Restart preserves records but
interrupts sessions. The development issuer trusts its application caller's human
confirmation; it is not a public identity boundary. Base-branch checks remain
remote observations and do not eliminate the GitHub read-to-merge race.
...
17. Reusable BYOK provider credentials remain in the self-hosting user's local
    credential store, while project-owned demonstration credentials remain in the
    managed deployment store. Only credential-holding services may resolve either;
    values are never stored in authority state, exposed to a runner or model, or
    returned publicly.
...
25. A directly confirmed normalized plan may grant persistent authority only for
...
27. Managed-demo client timestamps cannot control admission or settlement time.
    The queue samples the ledger clock at execution, derives queue deadlines from
    that evaluation, and applies expiry before settlement. Forged earlier or
    later client timestamps cannot extend the deployment window or reservation.
...
| Approval fatigue                           | A system repeatedly asks for routine commits, feature-branch pushes, or pull-request updates already included in a confirmed session plan, training the operator to approve reflexively | Persistent plan-bound grants, visible remaining authority, per-action membership checks, narrow selectors and limits, revocation, and step-up only at an actual scope or effect boundary                                                                                    |
