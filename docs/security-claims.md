...
| Guardian does not label an unverified runtime Enforced | Implemented | Assurance-state transition tests, missing and malformed evidence tests | `f058d41` adds strict evidence requirements, exact profile binding, and a fail-closed resolver; positive, expiry, and end-to-end transition evidence remains |
...
| Approval is bound to exact canonical request arguments and resource version | Goal | Canonicalization vectors, mutation property tests, resource-change rejection tests | `f058d41` adds versioned domain-separated RFC 8785 digests and equivalence/mutation properties; approval and resource-version binding remain |
...
| Unknown or malformed operations fail closed | Goal | Schema fuzzing and invalid-operation tests | `f058d41` covers strict mission, profile, assurance, and Guardian recommendation input; typed operation contracts and fuzzing remain |
