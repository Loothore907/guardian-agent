...
3. Guardian revalidates every answer, normalizes and clamps the completed draft to supported capabilities, and produces a candidate mission and versioned session profile defining tools, filesystem scope, network policy, time, volume, destinations, and side-effect limits. The exact worker-visible tool catalog is separately bound as a subset of those confirmed session capabilities.
...
5. Guardian displays the exact normalized candidate and consequences. Direct human confirmation of its digest is required before it becomes mission authority.
   A confirmed plan may also create a persistent, revocable grant for its enumerated workflow. Later concrete actions inside that grant do not require repeated confirmation, but each action is independently re-normalized and checked for plan membership, selectors, limits, expiry, revocation, and current policy before execution.
...
7. The worker performs local reasoning and coding and calls only its exact confirmed worker-tool subset. Guardian-mediated research or authenticated operations may exist at the broader session ceiling without becoming model-visible worker tools. Its model assignment is trusted deployment policy, never prompt-controlled session authority.
