...
- Product name: Agentic Guardian
- Canonical domain: `agentic-guardian.com` (registered; public deployment pending)
...
| 8 - Submit | Public repository, self-hosted test build, optional rate-limited judge demo, clean install, video, description, and feedback | Submission matches the tested tagged build |

The canonical domain supports documentation, downloads, release metadata, and the
competition presentation. It is not a customer credential-ingestion service.
Cloud-mode disclosures must state that selected context is sent to Nebius and
