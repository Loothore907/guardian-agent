...
| C5 - Tavily research gateway | In progress | Commit `2abdd53` and PR #12 pass the local implementation/evidence gate and remote build through the launcher, MCP catalog, session-bound local pipe, separate credential service, fixed Search adapter, budget, provenance, deterministic failures, and protected live call. PR security review and merge remain. |
