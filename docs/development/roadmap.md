...
| C6 - Authorization and GitHub broker     | In progress | Issue #13 and branch `codex/13-c6-durable-authorization-broker` are active. Local checkpoints `4a064a1`, `f341b96`, `f6087d4`, and `90da76b` capture W5 through W13: one-use journey supervision, strict broker IPC, separate Guardian action-risk IPC, credential-store-backed broker/research startup, fixed three-child composition, activated-session/connection-derived configuration, and exact CLI merge confirmation without widening the worker loop or combining provider credentials. The complete ordinary gate is green. Protected live worker and pre-activation evidence, successful GitHub automatic refresh, model-through-broker evidence, WebAuthn, platform parity/containment, competition-session startup and executable dispatch, remote CI, review, and branch merge remain. The checkpoint is still materially ahead of its October 2 target. |
