...
| C6 - Authorization and GitHub broker     | In progress | Issue #13 and branch `codex/13-c6-durable-authorization-broker` are active. Local checkpoint `0dd3323` captures authority, credential boundaries, pre-activation mission formation, W1 exact turns, W2 credential-safe session workspaces, and W3 exact one-tool/result execution. W4 now adds schema-v4 exact boundary events, minimized denial results, an inclusive five-minute/three-event ordinary threshold, immediate replay/binding revocation, and trusted-boundary interruption while preserving the one-request/two-turn lifecycle. Protected live worker and pre-activation evidence, successful GitHub automatic refresh, Guardian broker attachment, WebAuthn, platform parity/containment, remote CI, review, and branch merge remain. The checkpoint is still materially ahead of its October 2 target. |
