...
| C6 - Authorization and GitHub broker     | In progress | Issue #13 and branch `codex/13-c6-durable-authorization-broker` are active. Local checkpoints `4a064a1`, `f341b96`, `f6087d4`, and `90da76b` capture W5 through W13. Uncommitted W14 adds strict executable competition startup, activation-bound request construction, and an explicit separation between the broader confirmed mission/session ceiling and the narrower native-worker tool catalog. ADR-0029 makes the assembled live journey an architectural validation gate: validate Qwen, the worker, Tavily, Guardian, the no-effect assembly, and only then a disposable GitHub effect; correct contradicted design before considering feature expansion. The complete ordinary gate is green. W15-W18 record successful strict live Qwen mission dialogue plus Kimi final-response, exact typed-request, and sanitized denial-continuation behavior. The W18 denial was test-generated through production contracts, so assembled live authority/dispatcher containment remains unclaimed. ADR-0033 requires explicit model alternatives without silent fallback, while ADR-0034 separates future public locked and piloted judge experiences without widening authority. Live Tavily research and controlled hostile content are next. Repeated/draft-review Qwen evidence, later live boundaries, successful GitHub automatic refresh, model-through-broker evidence, WebAuthn, platform parity/containment, remote CI, review, and branch merge remain. The checkpoint is still materially ahead of its October 2 target. |
