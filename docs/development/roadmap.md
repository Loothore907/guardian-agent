...
| C6 - Authorization and GitHub broker     | In progress | Issue #13 and branch `codex/13-c6-durable-authorization-broker` are active. The product is now Agentic Guardian; `agentic-guardian.com` is registered and delegated to Cloudflare, while deployment remains pending. ADR-0008 locks local-first self-hosting and user-owned credentials, ADR-0009 selects a repository-bound GitHub App device flow, and ADR-0011 restores the external host agent as the worker. The credential slices now have provider-scoped contracts, deterministic isolation/rotation/revocation tests, hidden-input setup, fixed-origin read-only Nebius/Tavily/GitHub verification, executable interactive Windows enroll/status/revoke commands, deterministic GitHub device authorization with expiring access/refresh/metadata storage, callback-scoped broker resolution, dependency enforcement, and a locally probed Windows Credential Manager adapter. The SQLite spike, schema-v3 authority store, sole-owner authority service, authenticated role/session capability IPC, mandatory reference-supervisor injection, lower-assurance development approval issuer, objective-only terminal draft, server-derived fixed preview, digest-bound one-use CLI bootstrap, one-turn fake Guardian mission-brief assistant, launcher session creation, research reservation/settlement, connection/reference binding, minimized rejection chain, streamed fixed GitHub adapter, exact read/merge broker, and public squash-only `Loothore907/guardian-agent-demo` target pass or exist locally/remotely as applicable. The broker no longer imports SQLite. Protected device enrollment, exact-head PR read, and one-use squash merge pass against demo PR #1; the merge produced `16263e7a0e9bc81df55bac9b8413fc2256077a9d`. The deterministic reset planner and attended live reset created demo PR #2 at exact head `36251caf778466a7d08670ad8210375daf8a9bcb`, changing only the fixed fixture. The application-visible process/environment/model/SQLite/log/trace/audit/public-result secret corpus passes. ADR-0010, supervised authority/mission-brief children, credential-isolated Qwen/Nemotron adapters, strict separated projections, fail-closed Guardian precedence, authenticated current-model inventory, and the protected Qwen plus Super-to-Ultra live path pass. ADR-0016 and W1 add an exact-bound, expiring, one-use worker turn; deterministic fake and fixed-origin Nebius providers; strict final-or-pending-request output; credential-free provider projection; and fail-closed near-miss coverage without executing model-requested actions. ADR-0017 and W2 add an exact-confirmed, credential-screened, Guardian-owned session copy; sanitized no-remote Git baseline; post-confirmation source revalidation; persistent command binding; no automatic host writeback; and ownership-safe delete-on-close cleanup. ADR-0018 and W3 add exact request/execution/result binding, a dedicated replay-resistant worker authority role, atomic total/local-command metering, sanitized multiline result feedback, and a mandatory final second turn. W4 denial containment, protected live worker evidence, successful live GitHub automatic refresh, Guardian broker attachment, the WebAuthn issuer, platform peer/containment evidence, remote CI, review, and branch merge remain. The checkpoint is still materially ahead of its October 2 target. |
