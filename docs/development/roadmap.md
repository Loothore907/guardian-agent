...
| C6 - Authorization and GitHub broker     | In progress | Issue #13 and branch `codex/13-c6-durable-authorization-broker` are active. Local checkpoint `2de8d21` captures authority, credential boundaries, pre-activation mission formation, W1 exact turns, W2 credential-safe session workspaces, W3 exact one-tool/result execution, and W4 contained denial/revocation. Uncommitted W5 trusted orchestration now fixes the research-to-`scope_mismatch`-to-separately-approved-merge competition sequence without widening the worker loop. Protected live worker and pre-activation evidence, supervised W5 service/CLI attachment, successful GitHub automatic refresh, Guardian model-through-broker evidence, WebAuthn, platform parity/containment, remote CI, review, and branch merge remain. The checkpoint is still materially ahead of its October 2 target. |
