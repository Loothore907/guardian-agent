...
| C6 - Authorization and GitHub broker     | In progress | PR #14 exact-head squash-merged as `b69e9338d5464cac31d52cf8510256a2d9f21c33`; post-merge `main` CI passed and issue #13 remains open. W19-W23 prove protected Windows credential isolation, Tavily Search/Extract, deterministic and Nemotron no-effect denials, durable audit, exact-head read, and one separately authorized disposable squash merge. W24 fixes and actively probes the Linux database/socket permission sub-gate, including WAL/SHM sidecars, but does not prove OS peer identity. C6 is not Passed because intended-Linux peer identity, local credential resolution, service containment, and narrow GitHub read/merge remain open. Successful automatic refresh, WebAuthn, worker-generated dispatch, and the full coordinator remain bounded limitations or later slices, not evidence for closing that Linux criterion. |
