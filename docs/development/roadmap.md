...
- the provider-neutral native worker selected by ADR-0015 performs delegated
  work using the versioned coding-model assignment; future external-scaffold
  adapters may use the same contract, while neither Guardian model takes over
  the worker's coding or research loop;
